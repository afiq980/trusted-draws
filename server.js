...
async function initializeDbSchema(pool) {
  try {
...
function createApp(pool, options = {}) {
  const { initializeDb = false } = options;
  const app = express();
  app.use(helmet());
  ...
  app.get('/draw/:publicId', asyncHandler(async (req, res) => {
  ...
  app.get('/draw/:publicId/results', asyncHandler(async (req, res) => {
    const { publicId } = req.params;
  ...
  app.get('/manage/:adminToken', asyncHandler(async (req, res) => {
  ...
  app.post('/submit/:publicId', asyncHandler(async (req, res) => {
    const { publicId } = req.params;
  ...
  app.post('/draw/:adminToken/execute', asyncHandler(async (req, res) => {
    const { adminToken } = req.params;
  ...
  app.post('/draw/:adminToken/publish', asyncHandler(async (req, res) => {
    const { adminToken } = req.params;
    const drawResult = await pool.query('select * from draws where admin_token = $1', [adminToken]);
    if (!drawResult.rows.length) {
      return res.status(404).send('Draw not found');
  ...
  app.post('/verify/:publicId', asyncHandler(async (req, res) => {
    const { publicId } = req.params;
    const { entryText } = req.body;
    const drawResult = await pool.query('select * from draws where public_id = $1', [publicId]);
    if (!drawResult.rows.length) {
...
const app = createApp(pool, { initializeDb: process.env.NODE_ENV !== 'test' });

export default app;
export { createApp, buildPool, hashValue, buildVerificationKey };
